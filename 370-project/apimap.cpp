    ...
    mapboxToken_("pk.eyJ1IjoicmNhbHA5ODQ2IiwiYSI6ImNtZGllZnFodDBidGIybG9hNWl6MGo2bGkifQ.YkWGginaPMHEMwHFO92L4A") // ← Replace with your actual token
{
    ...
    buildingCoordinates_["Academic Hall"] = QGeoCoordinate(33.12812065219856, -117.15856711844148);
    buildingCoordinates_["Arts Building"] = QGeoCoordinate(33.130035070262586, -117.15802583677679);
    buildingCoordinates_["Extended Learning Building"] = QGeoCoordinate(33.13414127002338, -117.16011764848307);
    buildingCoordinates_["Kellogg Library"] = QGeoCoordinate(33.129085897084224, -117.15967655175587);
    buildingCoordinates_["Markstein Hall"] = QGeoCoordinate(33.12797627144677, -117.15782577448168);
    buildingCoordinates_["Science Hall I"] = QGeoCoordinate(33.12767420594281, -117.15873979957497);
    buildingCoordinates_["Science Hall II"] = QGeoCoordinate(33.130700120745125, -117.15794987104177);
    buildingCoordinates_["Social and Behavioral Sciences Building"] = QGeoCoordinate(33.13060042405003, -117.15724256704904);
    buildingCoordinates_["Soc & Behavioral Sci Bldg"] = QGeoCoordinate(33.13060042405003, -117.15724256704904);
    buildingCoordinates_["University Hall"] = QGeoCoordinate(33.128726937160486, -117.15855757758689);
    buildingCoordinates_["Viasat Engineering Pavilion"] = QGeoCoordinate(33.12722195391456, -117.15802726557422);
    buildingCoordinates_["Viasat Engineering Pav"] = QGeoCoordinate(33.12722195391456, -117.15802726557422);
}

QGeoCoordinate ApiMap::coordinateForBuilding(const QString &buildingName) const
{
    QString building = buildingName;
    building.replace(QRegularExpression("\\s*\\d+$"), "");
    if (buildingCoordinates_.contains(building)) {
        return buildingCoordinates_.value(building);
    }
...
void ApiMap::fetchWalkingRoute(const QList<QGeoCoordinate> &points)
{
    if (points.size() < 2) {
        //emit apiError("At least two coordinates are required to build a route.");
        emit noRoute();
        return;
    }
    ...
    for (const QGeoCoordinate &coord : points) {
        coordParts << QString::number(coord.longitude(), 'f', 6) + "," +
                          QString::number(coord.latitude(), 'f', 6);
    }

    QString coordinates = coordParts.join(";");
...
void ApiMap::showMarkerAt(const QGeoCoordinate &coord) {
    ...
    schedule_.append(coord); //add to schedule list
    fetchWalkingRoute(schedule_);
}

void ApiMap::removeMarkerAt(const QGeoCoordinate &coord){
    emit removeMarker(coord.latitude(), coord.longitude());
    schedule_.removeOne(coord); //remove from schedule list
    fetchWalkingRoute(schedule_);
}
...
void ApiMap::addInfoToMap(const QGeoCoordinate &coord, const QString &name, const QString &building){
    QString info = "•" + name + " (" + building + ")";
    emit addInfo(coord.latitude(), coord.longitude(), info);
}

void ApiMap::removeInfoMap(const QGeoCoordinate &coord, const QString &name, const QString &building){
    QString info = "•" + name + " (" + building + ")";
